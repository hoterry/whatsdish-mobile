...
const MenuSection = ({ restaurantId, restaurants }) => {
 ...
 console.log(`📌 Options in Group ${groupIndex + 1}:`, group.options); 
        ...
        if (menuItem.modifiers && menuItem.modifiers.length > 0) {
          console.log('🍕 Modifier Groups:', menuItem.modifiers);
          menuItem.modifiers.forEach((group, groupIndex) => {
            console.log(`⚡ Modifier Group ${groupIndex + 1}:`, group);
            console.log(`🔹 Modifier Items in Group ${groupIndex + 1}:`, group.modifiers);
          });
        }
