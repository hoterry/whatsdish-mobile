  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      ...
      if (__DEV__) {
        ...
        console.log('Fetching restaurants from:', API_URL); // 只在 DEV 環境中顯示
      }

      try {
        const response = await fetch(`${API_URL}/api/rn/merchants`); // 使用環境變量
        ...
        if (__DEV__) {
          console.log('Fetched data:', data); // 只在 DEV 環境中顯示
        }
