...
import ScrollHandler from './ScrollHandler'; // 导入 ScrollHandler 组件
...
const MenuSection = ({ restaurantId, restaurants }) => {
  ...
  const handleCategoryClick = (categoryName, index) => {
    ...
    // 计算该类别的位置，确保它居中
    const categoryWidth = 120; // 假设每个类别项的宽度是120px
    const offsetX = index * categoryWidth - (categoryWidth / 2);

    categoryListRef.current.scrollTo({
      x: offsetX, // 滚动到选中的类别，保持在中间
      animated: true,
    });

    flatListRef.current.scrollToIndex({
      index: index,  // 滚动到选中的类别位置
      animated: true,
    });
  };
...
const styles = StyleSheet.create({
  ...
  categoryText: {
    fontSize: 18,  // 调整字体大小
    color: '#333',
    lineHeight: 22, // 增加行高，确保文字底部不会被遮挡
