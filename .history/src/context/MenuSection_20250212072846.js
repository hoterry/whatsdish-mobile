...
const MenuSection = ({ restaurantId, restaurants }) => {
  ...
  const renderCategory = ({ item: category, index }) => (
    ...
    >
      ...
      {category.items.map((menuItem) => {
        // Check if the menu item has modifier_groups or option_groups
        ...
        return (
          ...
          >
            ...
            {/* Change button style and text based on whether the item has options */}
            <TouchableOpacity
              style={[styles.addButton, hasOptions && styles.optionButton]} // Add optionButton style if hasOptions
              onPress={() => {
                if (hasOptions) {
            ...
            >
              <Text style={[styles.addButtonText, hasOptions && styles.optionButtonText]}>
                {hasOptions ? (language === 'ZH' ? '選規格' : 'Option') : '+'}
              </Text>
            </TouchableOpacity>
          </TouchableOpacity>
        );
    ...
    <View style={styles.container}>
      ...
      >
        {groupedMenu.map((category, index) => (
          ...
          >
            <Text style={styles.categoryText}> {language === 'ZH' ? category.category_name_zh : category.category_name}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
  ...
  // Style for the add button when the item has options
  optionButton: {
    backgroundColor: 'rgba(0, 0, 0, 0.5)', // Green color for option button
    borderRadius: 8, // Rounded corners but not circular
    paddingVertical: 8, // Vertical padding for better button height
    paddingHorizontal: 15, // Horizontal padding
    justifyContent: 'center', // Center text
    alignItems: 'center', // Center text
    marginTop: 10, // Space between button and item
  },
  // Text style for the option button
  optionButtonText: {
