...
export const CartProvider = ({ children }) => {
  ...
  const getTotalPrice = (restaurantId) => {
    ...
    return cart.reduce((total, item) => {
      const basePrice = item.price_in_cents ? item.price_in_cents / 100 : 0; // 基本價格
      const totalPrice = (basePrice + item.selectedOption?.price / 100 || 0 + item.selectedModifiers?.reduce((sum, modifier) => sum + (modifier.price / 100 || 0), 0) || 0) * item.quantity;
      
      return total + totalPrice; // 返回總價格
    }, 0);
  };
