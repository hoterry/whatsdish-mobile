    ...
    if (restaurant && restaurant.formatted_address) {
      const fetchLatLng = async () => {
        ...
        if (!location) {
          ...
          console.log('Formatted Address:', restaurant.formatted_address); // 打印完整的地址
        ...
        // 如果仍然没有找到位置，输出错误
        if (!location) {
          console.error('Unable to find location after all attempts.');
        } else {
...
const extractPostalCode = (address) => {
    const postalCodeRegex = /[A-Z]\d[A-Z] \d[A-Z]\d$/; // 匹配加拿大邮政编码格式
    const match = address.match(postalCodeRegex);
    if (match) {
      console.log('Extracted Postal Code:', match[0]);  // 打印提取到的邮政编码
