...
const renderCategory = ({ item: category, index }) => (
  ...
  >
    <Text style={styles.categoryHeader}>
      {getText(category.category_name, category.category_name_zh)} 
    </Text>
      ...
      <View key={menuItem.id} style={styles.menuItem}>
        ...
        <View style={styles.info}>
          <Text style={styles.name}>
            {getText(menuItem.name, menuItem.name_zh)} 
          </Text>
          <Text style={styles.description} numberOfLines={2} ellipsizeMode="tail">
            {getText(menuItem.description, menuItem.description_zh)} 
          </Text>
        ...
        {groupedMenu.map((category, index) => (
          ...
          >
            <Text style={styles.categoryText}>
              {getText(category.category_name, category.category_name_zh)} {/* 动态显示分类名称 */}
            </Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      {cartItemCount > 0 && (
        <TouchableOpacity style={styles.viewCartButton} onPress={handleViewCart}>
          <Text style={styles.viewCartButtonText}>
            {getText(`View Cart (${cartItemCount})`, `查看购物车 (${cartItemCount})`)} {/* 动态显示购物车按钮文本 */}
          </Text>
        </TouchableOpacity>
      )}
    </View>
