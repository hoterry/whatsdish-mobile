...
const MenuSection = ({ restaurantId, restaurants }) => {
  ...
  const handleDataFetched = (data) => {
    console.log('Fetched Data in MenuSection:', data);  // 查看原始數據
    
    if (!data || !data.menu || !data.menu.categories) {
    ...
    // 直接使用 API 返回的 categories 來設定 groupedMenu
    const grouped = data.menu.categories.map(category => ({
      category_name: category.name,
      category_name_zh: category.name_zh || category.name, // 避免沒有中文名稱的情況
      items: category.items || [], // 確保 items 是陣列
    }));
