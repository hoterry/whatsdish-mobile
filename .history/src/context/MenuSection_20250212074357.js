...
const MenuSection = ({ restaurantId, restaurants }) => {
          ...
          >
            <View style={styles.info}>
              <Text style={styles.name}>
                {language === 'ZH' ? menuItem.name_zh : menuItem.name}
              </Text>
            ...
            {/* Change button text and behavior based on whether the item has options */}
            <TouchableOpacity
  ...
  container: {
    flex: 1,
    paddingBottom: 70, // 保證底部有足夠的空間，避免被按鈕遮擋
  },
  separator: {
    height: 1,
    backgroundColor: '#ddd',
  ...
  contentContainer: {
    paddingBottom: 70, // 確保內容區域不會被按鈕遮擋
  },
  description: {
    fontSize: 16,
    color: '#555',
  ...
  viewCartButton: {
    position: 'absolute', // 固定位置在底部
    bottom: 20, // 距離底部 20 單位
    left: 20,
    right: 20,
