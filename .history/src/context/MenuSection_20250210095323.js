...
import { useCart } from './CartContext';  // 导入 useCart 钩子
...
useEffect(() => {
  const fetchMenu = async () => {
    try {
      ...
      console.log("Fetched menu data:", data); // 输出从 API 返回的数据
  ...
  const handleAddToCart = (item) => {
    console.log("Add to Cart clicked");  // 确认是否触发了点击事件r
    console.log(restaurantId);  // 打印查看 restaurantId 是否正确
    addToCart(restaurantId, item); // 添加餐厅 ID
  };
  


  const handleViewCart = () => {
    navigation.navigate('Cart', { restaurantId, restaurants }); // 传递 restaurantId 和 restaurants
  };
  ...
  <View key={menuItem.id} style={styles.menuItem}>
    ...
    <TouchableOpacity
      ...
      onPress={() => handleAddToCart(menuItem)} // 点击按钮时调用 handleAddToCart
    >
