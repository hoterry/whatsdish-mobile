...
const MenuSection = ({ restaurantId, restaurants }) => {
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        const response = await fetch(`http://10.0.0.7:5000/menu?restaurant_id=${restaurantId}`);
        if (!response.ok) {
  ...
  // Select the first category when the menu is loaded
  useEffect(() => {
  ...
  // Handle scroll to change selected category
  const handleScroll = (event) => {
    ...
    categoryListRef.current.scrollTo({ x: categoryIndex * 120, animated: true });
  };

  // Track category heights to determine which category is visible
  ...
  // Handle category selection on click
  ...
  const cartItemCount = getTotalItems(restaurantId); // Get current cart item count for the restaurant
