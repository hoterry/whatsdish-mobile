...
const MenuSection = ({ restaurantId, restaurants }) => {
  ...
  const handleDataFetched = (data) => {
    ...
    if (!data || !data.data || !data.data.menu || !Array.isArray(data.data.menu.categories)) {
      console.error('Invalid menu data structure:', data);
      return;
    }
    console.log('Categories Data:', JSON.stringify(data.data.menu.categories, null, 2));
    setMenu(data.data.menu);
  
    // 直接使用 API 返回的 categories
    const grouped = data.data.menu.categories.map(category => ({
      category_name: category.name,
      category_name_zh: category.name_zh || category.name, // 如果沒有中文名稱則使用英文名稱
      items: category.items || [], // 確保 items 存在
    }));
  ...
  const renderCategory = ({ item: category, index }) => (
    ...
    >
      ...
      {category.items.map((menuItem) => {
        console.log('🔍 Menu Item in Menu Section:', menuItem); 
  
        // ✅ Log 選項 (Options)
        if (menuItem.option_groups && menuItem.option_groups.length > 0) {
          console.log('🛠 Option Groups:', menuItem.option_groups);
          menuItem.option_groups.forEach((group, groupIndex) => {
            console.log(`🎯 Option Group ${groupIndex + 1}:`, group);
 console.log(`📌 Options in Group ${groupIndex + 1}:`, group.options); 
          });
        }
        ...
        if (menuItem.modifier_groups && menuItem.modifier_groups.length > 0) {
          ...
          menuItem.modifier_groups.forEach((group, groupIndex) => {
            console.log(`⚡ Modifier Group ${groupIndex + 1}:`, group);
            console.log(`🔹 Modifier Items in Group ${groupIndex + 1}:`, group.modifier_items);
          });
        }
