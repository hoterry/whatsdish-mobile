    ...
    <View style={styles.container}>
      ...
      <FlatList
        ...
        contentContainerStyle={{
          paddingBottom: 700 * scaleHeight,  // 这个确保了 FlatList 内容底部有足够空间
          paddingBottom: 80 * scaleHeight,   // 留出空间给按钮
        }}
      />
  
      {/* View Cart 按钮位于 FlatList 内部的底部 */}
      {cartItemCount > 0 && (
        <TouchableOpacity
          style={[styles.viewCartButton, { bottom: 20 * scaleHeight }]}  // 确保按钮浮动到底部
          onPress={handleViewCart}
        >
          <Text style={styles.viewCartButtonText}>
            {language === 'ZH' ? '查看购物车' : 'View Cart'} ({cartItemCount})
          </Text>
        </TouchableOpacity>
      )}
