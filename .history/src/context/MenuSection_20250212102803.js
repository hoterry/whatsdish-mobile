const MenuSection = ({ restaurantId, restaurants }) => {
  ...
  const { handleCategoryClick } = ScrollHandler({ groupedMenu, setSelectedCategory });
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {groupedMenu.map((category) => (
          <TouchableOpacity
            ...
            style={[
              styles.categoryItem,
              selectedCategory === category.category_name && styles.selectedCategory,
            ]}
      ...
      <ScrollView style={styles.menuContent}>
        {groupedMenu.map((category) => {
          if (category.category_name !== selectedCategory) return null;
          return (
            <View style={styles.categorySection} key={category.category_name}>
...
const styles = StyleSheet.create({
  ...
  viewCartButton: {
    position: 'absolute',
    left: 22 * scaleWidth,
    bottom: 20 * scaleHeight, // 确保按钮出现在底部
    backgroundColor: '#000',
