...
const DeliveryOptions = ({
    ...
    const handlePickupTimeConfirm = (time) => {
      ...
      const formatSelectedTime = (time) => {
        // 檢查是否選擇了 "scheduled" 且未選擇時間
        if (pickupOption === 'scheduled' && !time) {
          return 'Select a time'; // 如果選擇了 Schedule 且時間為 null，顯示 "Select a time"
        }
        if (!time) return '15-25min'; // 如果沒有選擇時間，顯示 Standard 的預設時間
