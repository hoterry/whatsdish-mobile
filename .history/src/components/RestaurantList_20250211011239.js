...
const RestaurantList = ({ restaurants }) => {
  ...
  // Translate function for pickUp and delivery
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      {restaurants.map((item) => {
        // Print the discount field to the console for debugging
        console.log('Discount for restaurant', item.name, ':', item.discount);
        ...
        return (
          <View key={item.restaurant_id.toString()} style={styles.restaurantCard}>
            <Pressable onPress={() => navigation.navigate('Details', { restaurant: item, restaurants })}>
              {/* Render the discount based on language */}
              {item.discount && (
                <View style={styles.discountTag}>
                  <Text style={styles.discountText}>
                    {item.discount[language] || item.discount.EN} 
                  </Text>
                </View>
              )}
