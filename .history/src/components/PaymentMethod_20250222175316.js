...
// 在 PaymentScreen 中的 onSaveCard 函數，假設你在這裡處理儲存卡片的邏輯
const onSaveCard = async (cardInfo) => {
  try {
    ...
    const newCard = await response.json(); // 假設返回的資料是新增的卡片
    console.log('Card saved successfully:', newCard);

    // 更新已儲存的卡片
    setSavedCards(prevCards => [...prevCards, newCard]); // 新卡片加入陣列
          ...
          onPress: async () => {
            ...
            if (response.ok) {
              ...
              if (result.success) {
                ...
                // 恢復被刪除卡片
                setSavedCards(prevCards => [...prevCards, { _id: cardId }]);
