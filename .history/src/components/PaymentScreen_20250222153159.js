...
const PaymentScreen = ({ onClose, onSaveCard }) => {
  ...
  const handleSave = () => {
    ...
    // 1. Remove spaces from card number and ensure expiration date is in YYMM format
    ...
    // 2. Validation: Ensure card number is 16 digits, expiry date is 4 digits, and CVV is 3 digits
    if (formattedCardNumber.length === 16 && formattedExpirationDate.length === 4 && cvv.length === 3) {
      ...
      const cardInfo = {
        ...
        ip: '50.64.173.103', // Replace with dynamic IP if needed
