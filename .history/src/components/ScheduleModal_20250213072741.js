  ...
  const generateTimeOptions = () => {
    ...
    for (let hour = 11; hour <= 22; hour++) {
      for (let minute of ['00', '30']) {
        const period = hour >= 12 ? 'pm' : 'am'; // 變成小寫
...
const styles = StyleSheet.create({
  ...
  dateOption: {
    paddingVertical: 8, // 減少高度
    paddingHorizontal: 16, // 增加一點寬度
    borderWidth: 2,
    borderColor: '#ccc',
    borderRadius: 12,
    alignItems: 'center',
  ...
  timeText: {
    fontSize: 18, // 時間字體變大
    fontWeight: 'normal', // 預設非粗體
  },
  selectedTimeText: {
    fontWeight: 'bold', // 被選中的時間才粗體
  },
  radio: {
    width: 20,
    height: 20,
  ...
  selectedRadio: {
    borderColor: 'black',
    backgroundColor: 'black', // 被選中的時間圓點全黑
  },
  confirmButton: {
