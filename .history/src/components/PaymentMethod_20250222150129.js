...
const PaymentMethod = ({ onSelectMethod, onClose }) => {
  ...
  const handleSaveCard = async (cardInfo) => {
    ...
    try {
      ...
      // 2️⃣ 儲存 `data_key` 到應用的後端
      console.log('⏳ Saving card to app database...');
      const appResponse = await fetch('https://dev.whatsdish.com/api/profile/payment-methods', {
        ...
        body: JSON.stringify({
          type: cardInfo.type,
          data_key: dataKey,
          masked_pan: `**** **** **** ${cardInfo.last4}`,
          expdate: cardInfo.expiry,
          is_default: false,
        }),
      });
      ...
      console.log('📨 App API Response Data:', appData);
      ...
      console.log('✅ Card saved successfully in app database.');
