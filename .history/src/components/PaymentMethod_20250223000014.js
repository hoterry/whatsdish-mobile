...
const PaymentMethod = ({ onSelectMethod, onClose, onRemoveCard }) => {
  ...
  const onSaveCard = async (cardInfo) => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/payments/m/cof`,  {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
          ...
          },
          {
            ...
            onPress: async () => {
              const response = await fetch(
                `${API_URL}/api/profile/payment-methods/${cardId}`,
                {
                  method: 'DELETE',
                  headers: {
                    'Content-Type': 'application/json',
              ...
              return (
                <View key={index} style={styles.savedCardContainer}>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      if (__DEV__) {
                        console.log('[Payent Method Log] Card selected:', card);
                      }
                      onSelectMethod(card);
                      onClose();
                    }}
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      if (__DEV__) {
                        console.log('[Payent Method Log] Removing card with ID:', card._id);
                      }
