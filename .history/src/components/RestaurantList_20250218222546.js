...
const RestaurantList = ({ restaurants }) => {
  ...
  const getDiscountText = (discount) => {
    ...
    const parsedDiscount = JSON.parse(discount); // 解析 JSON 字符串
    const discountText = parsedDiscount[language] || parsedDiscount.EN; // 默认显示英文
    console.log('Discount info for restaurant:', discountText); // 打印折扣信息
    return discountText;
  };
      ...
      {restaurants.map((item) => (
        <View key={item.gid} style={styles.restaurantCard}>
          <Pressable onPress={() => navigation.navigate('Details', { restaurant: item, restaurants })}>
            <Image source={{ uri: item.banner_url }} style={styles.restaurantBanner} />
