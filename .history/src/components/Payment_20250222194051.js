...
const Payment = () => {
  ...
  const [savedCards, setSavedCards] = useState([]); // 存儲所有信用卡
  ...
  const handleSelectMethod = (method) => {
    ...
    setModalVisible(false); // 關閉 modal，但不重新 fetch，保持選擇的信用卡
  };

  const handleRemoveCard = async () => {
    await fetchSavedCards(); // 重新獲取信用卡資料
  };
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              <PaymentMethod
                ...
                onRemoveCard={handleRemoveCard} // 傳遞 onRemoveCard 回調
              />
            </View>
          </View>
        </Modal>
