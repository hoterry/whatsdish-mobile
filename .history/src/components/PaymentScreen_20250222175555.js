...
const PaymentScreen = ({ onClose, onSaveCard }) => {
  ...
  const handleSave = () => {
    const formattedCardNumber = cardNumber.replace(/\s/g, ''); // 移除空格，確保是16位數字
    const [month, year] = expirationDate.split(' '); // 按空格拆分
    const formattedExpirationDate = `${year}${month}`; // 轉換為YYMM格式
    const ip = '50.64.173.103';  // 替換為動態 IP，如有需要
    
    const cardInfo = {
      pan: formattedCardNumber,  // 確保是16位數字字符串
      ip: ip.trim(), // 去除 IP 字符串中的空格
      expiry_date: formattedExpirationDate,  // 使用YYMM格式的過期日期
    };
