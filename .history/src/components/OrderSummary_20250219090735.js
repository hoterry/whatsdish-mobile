...
const OrderSummary = ({ cart, subtotal, showDeliveryFee, deliveryFee, taxes, totalPrice }) => {
  ...
  const renderOrderItem = (item) => {
    ...
    const options = item.option_groups
      ...
      const optionPrice = item.selectedOption ? (item.selectedOption.price ? item.selectedOption.price / 100 : 0) : 0;
      const modifiersPrice = item.selectedModifiers?.reduce((sum, modifier) => sum + (modifier.price / 100 || 0), 0) || 0; // 處理 modifiers 的價格
    ...
    // 計算選項和修飾符的總價格
    const optionsTotal = options.reduce((sum, option) => sum + option.price /100, 0);
    const modifiersTotal = modifiers.reduce((sum, modifier) => sum + modifier.price /100, 0);
