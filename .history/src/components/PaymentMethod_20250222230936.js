...
const PaymentMethod = ({ onSelectMethod, onClose, onRemoveCard }) => {
  ...
  const fetchSavedCards = async () => {
    try {
      ...
      if (!token) {
        if (__DEV__) console.error('No token found'); // 只在開發環境中顯示
        return;
      }
      ...
      if (!response.ok) {
        if (__DEV__) console.error('Failed to fetch saved cards'); // 只在開發環境中顯示
        return;
      }
      ...
      if (__DEV__) console.error('Error fetching saved cards:', error); // 只在開發環境中顯示
    }
  };
  ...
  const onSaveCard = async (cardInfo) => {
    try {
      ...
      if (!response.ok) {
        ...
        if (__DEV__) console.error('Failed to save card:', errorData); // 只在開發環境中顯示
        alert(language === 'ZH' ? '卡片儲存失敗' : 'Card save failed');
        return;
      }

      const newCard = await response.json();
      if (__DEV__) console.log('Card saved successfully:', newCard); // 只在開發環境中顯示
      setSavedCards(prevCards => [...prevCards, newCard]);
      fetchSavedCards();
      alert(language === 'ZH' ? '卡片已儲存' : 'Card saved');
    } catch (error) {
      if (__DEV__) console.error('Error saving card:', error); // 只在開發環境中顯示
      alert(language === 'ZH' ? '卡片儲存失敗' : 'Card save failed');
    }
  };

  const handleRemoveCard = async (cardId) => {
    if (__DEV__) console.log(`Attempting to remove card with ID: ${cardId}`); // 只在開發環境中顯示
    try {
      const token = await SecureStore.getItemAsync('token');
      if (!token) {
        if (__DEV__) console.error('No token found'); // 只在開發環境中顯示
      ...
      if (__DEV__) console.log('Token retrieved:', token); // 只在開發環境中顯示
      Alert.alert(
        ...
        [
          {
            ...
            onPress: () => if (__DEV__) console.log('Cancel pressed'), // 只在開發環境中顯示
            style: 'cancel',
          },
          {
            ...
            onPress: async () => {
              ...
              if (response.ok) {
                ...
                if (result.success) {
                  if (__DEV__) console.log(`Card with ID ${cardId} successfully removed`); // 只在開發環境中顯示
                  alert(language === 'ZH' ? '卡片已移除' : 'Card removed');

                  await fetchSavedCards();
                  onRemoveCard();
                } else {
                  if (__DEV__) console.error('Failed to remove card'); // 只在開發環境中顯示
      ...
      if (__DEV__) console.error('Error removing card:', error); // 只在開發環境中顯示
      alert(language === 'ZH' ? '移除卡片失敗' : 'Failed to remove card');
    }
  };
  ...
  const getCardType = (maskedPan) => {
    if (!maskedPan) {
      if (__DEV__) console.error('Invalid masked_pan:', maskedPan); // 只在開發環境中顯示
      return 'Unknown';
    }
              ...
              return (
                <View key={index} style={styles.savedCardContainer}>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      if (__DEV__) console.log('Card selected:', card); // 只在開發環境中顯示
                      onSelectMethod(card);
                      onClose();
                    }}
                  >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      if (__DEV__) console.log('Removing card with ID:', card._id); // 只在開發環境中顯示
