...
const PaymentScreen = ({ onClose, onSaveCard }) => {
  ...
  const handleSave = () => {
    console.log('Saving Card Information...');  // Log save operation
    console.log('Card Number:', cardNumber);
    console.log('Expiration Date:', expirationDate);
    console.log('CVV:', cvv);
    console.log('Postal Code:', postalCode);
  
    // Remove spaces from card number and ensure expiration date is in YYMM format
    const formattedCardNumber = cardNumber.replace(/\s/g, ''); // Remove spaces
    const [month, year] = expirationDate.split(' / '); // Split by '/'
    const formattedExpirationDate = `${year}${month}`; // Convert to YYMM format
  
    // Validation: Card number should be 16 digits, expiry date should be 4 digits
    if (formattedCardNumber.length === 16 && formattedExpirationDate.length === 4 && cvv.length === 3) {
      const last4 = formattedCardNumber.slice(-4);
      const cardInfo = {
        pan: formattedCardNumber,  // Ensure pan is 16 digits only
        ip: '50.64.173.103', // Add the IP address here, replace with actual dynamic value if needed
