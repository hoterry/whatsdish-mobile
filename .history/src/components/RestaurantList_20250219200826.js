...
import Distance from '../context/DistanceCalculator'; // 引入刚刚创建的 Distance 组件
...
const RestaurantList = ({ restaurants, userLocation }) => {
  ...
  const [distanceLoading, setDistanceLoading] = useState(true);  // 跟踪距离是否加载完成

  // 监听餐厅数据变化，检查每个餐厅是否都加载了距离
  useEffect(() => {
    if (restaurants && Array.isArray(restaurants.data)) {
      // 检查所有餐厅的距离是否已加载完
      const allLoaded = restaurants.data.every(item => item.distance !== undefined);
      setDistanceLoading(!allLoaded);
    }
  }, [restaurants]);

  // 如果距离未加载完成，则显示加载提示
  if (distanceLoading) {
    return <Text>Loading distances...</Text>;
  }
