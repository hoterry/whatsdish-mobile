...
const ScheduleModal = ({
  ...
  const [selectedDate, setSelectedDate] = useState(initialDateTime ? initialDateTime.date : ''); // Initialize as empty or previously selected date
  const [selectedTime, setSelectedTime] = useState(initialDateTime ? new Date(initialDateTime.time) : null); // Initialize with the previously selected time if available

  useEffect(() => {
    if (isVisible) {
      // If the modal is opened, retain the previously selected date and time
      if (initialDateTime) {
        setSelectedDate(initialDateTime.date);
        setSelectedTime(new Date(initialDateTime.time));
      } else {
        setSelectedDate(''); // Reset if no initial date and time
