  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      {restaurants.map((item) => (
        <View key={item.restaurant_id} style={styles.restaurantCard}>
          <Pressable onPress={() => navigation.navigate('Details', { restaurant: item })}>
            {/* 显示折扣 */}
            {item.discount && (
              <View style={styles.discountTag}>
                <Text style={styles.discountText}>
                  {item.discount[language] || item.discount.en} {/* 显示当前语言的折扣 */}
                </Text>
              </View>
            )}
