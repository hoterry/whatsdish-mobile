...
const Rankings = () => {
  const [activeIndex, setActiveIndex] = useState(0); // 当前选中的标题索引
  const flatListRef = useRef(null); // 引用 FlatList

  // 标题切换时触发
  const handleTitlePress = (index) => {
    setActiveIndex(index); // 更新标题索引
    flatListRef.current.scrollToOffset({ offset: index * SCREEN_WIDTH, animated: true }); // 滑动到对应内容
  };

  // 内容滑动时触发
  const handleContentScroll = (event) => {
    const offsetX = event.nativeEvent.contentOffset.x;
    const index = Math.round(offsetX / SCREEN_WIDTH); // 根据滑动位置计算索引
    setActiveIndex(index); // 更新标题索引
  };
