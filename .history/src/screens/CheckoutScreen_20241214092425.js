...
import { FontAwesome } from 'react-native-vector-icons'; // 导入 FontAwesome 图标
...
const { restaurantId, restaurants } = route.params;  // 获取传递过来的 restaurantId 和 restaurants
...
const [tipPercentage, setTipPercentage] = useState(0);  // 小费百分比
const [tipCustom, setTipCustom] = useState("");           // 自定义小费金额
const [tipAmount, setTipAmount] = useState(0);            // 最终小费金额（根据百分比或输入）
const { orderHistory, addOrder } = useOrderHistory();
const [restaurantInfo, setRestaurantInfo] = useState(null);

useEffect(() => {
  // 查找与 restaurantId 匹配的餐厅
...
}, [restaurantId, restaurants]);  // 在 restaurantId 或 restaurants 变化时重新执行


useEffect(() => {
  console.log('Received restaurants data:', restaurants);  // 输出 restaurants 数据
}, [restaurants]); // 依赖 restaurants，当它发生变化时，log 会重新执行

const generateOrderId = () => {
  const timestamp = Date.now(); // 获取当前时间戳
  const random = Math.floor(Math.random() * 10000); // 生成一个 0 到 9999 的随机数
  return `ORD-${timestamp}-${random}`; // 组合时间戳和随机数生成订单ID
};
...
const cart = cartItems[restaurantId] || []; 
  // If cart is not available, show loading or empty message
  if (cart.length === 0) {
    return <Text>Your cart is empty</Text>;
  }

  useEffect(() => {
    console.log(cart); // 检查 cart 是否正确
  }, [cart]);  // 依赖项改为 cart
      ...
      [
        {
          ...
          onPress: () => {
            ...
            clearCart();
            addOrder(orderHistory);  // 添加订单到订单历史
            console.log('Order Data on checkoutttttttt:', orderData); // Now you can log
