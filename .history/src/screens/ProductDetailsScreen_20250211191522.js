...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const handleAddToCart = () => {
    if (selectedOption) {
      ...
      // Create a unique ID based on menuItem.id, selectedOption, and selectedModifiers
      const itemWithOptionAndModifiers = {
        ...menuItem,
        selectedOption,
        selectedModifiers,
        price: selectedOption.price + totalModifiersPrice,  // Include modifier prices in total
        uniqueId: `${menuItem.id}-${selectedOption.id}-${selectedModifiers
          .map(m => m.id).join('-')}`,  // Create a unique ID using the selected option and modifiers
      };
