...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const handleAddToCart = () => {
    if (selectedOption) {
      ...
      // Generate a unique ID by including the selected modifiers in the combination
      const uniqueId = `${menuItem.id}-${selectedOption?.id || 'default'}-${selectedModifiers.map(m => m.id).join('-')}`;
      ...
      const itemWithOptionAndModifiers = {
        ...
        uniqueId,  // Ensure the unique ID is set here
      };
