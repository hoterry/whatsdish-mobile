  ...
  const sendVerificationCode = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp(
        { phone: phone, password: 'randompassword' }, // 使用一个随机密码
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {isCodeSent && (
        <>
          ...
          />
          <Text style={styles.countdownText}>Time left: {countdown}s</Text> {/* 显示倒计时 */}
        </>
      )}
      ...
      {!isCodeSent ? (
        ...
        <TouchableOpacity style={styles.button} onPress={verifyCode} disabled={loading || countdown === 0}>
          {loading ? (
            <ActivityIndicator color="#fff" />
          ) : (
