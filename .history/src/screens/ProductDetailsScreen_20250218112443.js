...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  // Prepare the modifiers with localized names
  const modifiers = (menuItem.modifier_groups || []).flatMap(group =>
    group.modifier_items
      ? group.modifier_items.map(item => {
          ...
          console.log(`Modifier name for language ${language}:`, name); // Log the selected name
          return {
            id: item.id,
            name: name,
            price: item.price,
  ...
  const handleAddToCart = () => {
    ...
    const selectedOptionName = selectedOption ? selectedOption.name : (language === 'ZH' ? '未選擇' : 'No option selected');
    ...
    const itemWithOptionAndModifiers = {
      ...menuItem,
      selectedOption: { ...selectedOption, name: selectedOptionName }, // Ensure name is set
      selectedModifiers,
      price: currentPrice,
      uniqueId: `${menuItem.id}-${selectedOptionId}-${selectedModifiersIds}-${Date.now()}`,
    };
  
    console.log("Item to be added to cart:", itemWithOptionAndModifiers);  // Log the item being added
  
    addToCart(restaurants, restaurantId, itemWithOptionAndModifiers, 1);
  
    // Confirm if navigation works after adding to cart
    console.log("Navigating back after adding to cart...");
    navigation.goBack();
  };
      ...
      <View key={group.id}>
        ...
        {group.modifier_items.map((modifier) => (
          <TouchableOpacity
            ...
            onPress={() => {
              console.log("Toggling modifier:", modifier);  // Log the modifier being toggled
