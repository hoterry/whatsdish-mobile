...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const toggleModifier = (modifier, groupId) => {
    // Find the modifier group to check its maxAllowed
    const modifierGroup = menuItem.modifier_groups.find(group => group.id === groupId);
    const maxAllowed = modifierGroup?.maxAllowed || 0;
  
    console.log("Toggling modifier:", modifier); // Log each modifier toggle
  
    // Check if adding the modifier would exceed the allowed limit
    setSelectedModifiers((prevModifiers) => {
      const alreadySelected = prevModifiers.some((m) => m.id === modifier.id);
      let newModifiers = [...prevModifiers];
  
      if (alreadySelected) {
        // If modifier is already selected, remove it
        newModifiers = newModifiers.filter((m) => m.id !== modifier.id);
      } else if (newModifiers.length < maxAllowed) {
        // If we haven't reached the limit, add the modifier
        newModifiers.push(modifier);
      } else {
        // Show an alert or message if limit is reached
        alert(`You can select up to ${maxAllowed} ${modifierGroup.name}`);
      }
            ...
            <View key={group.id}>
              ...
              {group.modifiers.map((modifier) => (
                <TouchableOpacity
                  ...
                  onPress={() => toggleModifier(modifier, group.id)} 
                  disabled={selectedModifiers.length >= group.maxAllowed && !selectedModifiers.some((m) => m.id === modifier.id)}
                >
