...
export default function LoginScreen({ setIsAuthenticated }) {
  ...
  const handleSendCode = async () => {
    setErrorMessage('');
    if (phoneNumber.some(digit => !digit)) {
    ...
    try {
      const response = await fetch('http://10.0.0.7:5000/api/send-code', {
        ...
        body: JSON.stringify({ phoneNumber: phoneNumber.join('') }),
      });
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Stored userPhoneNumber:', phoneNumber.join(''));
      ...
      <View style={styles.phoneContainer}>
        {phoneNumber.map((digit, index) => (
          <TextInput
            ...
            maxLength={1}
            ref={(ref) => (phoneInputs.current[index] = ref)}
