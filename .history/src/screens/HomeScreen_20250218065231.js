...
const HomeScreen = () => {
  ...
  useEffect(() => {
    ...
    const getLocation = async () => {
      ...
      if (results.length > 0) {
        // 获取地址组件，包括街道号、街道名、城市、区域、国家等
        const { street, name, city, region, country, postalCode } = results[0];
  
        // 构建完整地址
        const userLocation = `${street ? street : ''} ${name ? name : ''}, ${city}, ${region}, ${country}${postalCode ? `, ${postalCode}` : ''}`;
        
        console.log('User location:', userLocation); // 打印完整地址信息
        setLocation(userLocation); // 保存完整地址
      } else {
