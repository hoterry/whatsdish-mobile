...
import { View, Text, StyleSheet, TouchableOpacity, Image, ScrollView, SafeAreaView, Pla } from 'react-native';
...
const HomeScreen = () => {
  ...
  const [filteredRestaurants, setFilteredRestaurants] = useState([]); // 新增過濾後的餐廳狀態
  ...
  const handleDataFetched = (data) => {
    setRestaurants(data);
    setFilteredRestaurants(data); // 初始化時顯示所有餐廳
  };

  const handleSearchChange = (text) => {
    setSearchQuery(text); // 更新搜尋詞
    filterRestaurants(text); // 根據搜尋詞過濾餐廳
  };

  // 搜尋過濾餐廳
  const filterRestaurants = (query) => {
    if (!query) {
      setFilteredRestaurants(restaurants); // 如果搜尋詞為空，顯示所有餐廳
    } else {
      const filtered = restaurants.filter((restaurant) =>
        restaurant.name.toLowerCase().includes(query.toLowerCase()) // 根據餐廳名稱過濾
      );
        ...
        <SafeAreaView style={styles.safeArea}>
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.restaurantSection}>
              ...
              <RestaurantList restaurants={filteredRestaurants} /> {/* 顯示過濾後的餐廳 */}
            </View>
          </ScrollView>
        </SafeAreaView>
      )}
