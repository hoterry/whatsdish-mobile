...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, TextInput ,useNavigation } from 'react-native';
...
const AccountScreen = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      navigation.reset({
        index: 0,  // 設定為重設為只有一個導航堆棧
        routes: [{ name: 'Login' }], // 使用 reset 將導航堆棧重置到 Login
      });
    } catch (error) {
