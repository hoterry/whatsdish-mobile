...
function CartScreen() {
  ...
  const handleIncreaseQuantity = (uniqueId) => {
    const item = safeCart.find(item => item.uniqueId === uniqueId); // 使用 uniqueId 查找
    if (item) {
  ...
  const handleDecreaseQuantity = (uniqueId) => {
    ...
    if (item && item.quantity > 1) {
      updateQuantity(restaurantId, item.uniqueId, item.quantity - 1); // 使用 uniqueId 更新数量
    }
  };

  const renderCartItem = (item) => {
    console.log('Cart Item:', JSON.stringify(item, null, 2)); // 打印整个 item
    console.log('Selected Option:', item.selectedOption?.name); // 确保 selectedOption 存在
