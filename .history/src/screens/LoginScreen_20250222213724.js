...
import Constants from 'expo-constants'; // 確保引入了 Constants
...
export default function LoginScreen({ setIsAuthenticated }) {
  ...
  const { API_URL } = Constants.expoConfig.extra; // 獲取 API_URL

  // 只有在開發模式下才輸出 log
  if (__DEV__) {
    console.log('__DEV__:', __DEV__);
    console.log('Current Environment:', process.env.NODE_ENV); // 日誌顯示當前環境
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/send-code`, { // 使用環境變量
        method: 'POST',
        headers: {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/verify-code`, { // 使用環境變量
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleCodeChange = (text, index) => {
    ...
    if (__DEV__) {
      console.log('Code input changed:', newCode.join(''));
    }
  };
  ...
  useEffect(() => {
    if (code.every(digit => digit !== '') && isCodeSent) {
      if (__DEV__) {
        console.log('All digits entered, attempting verification');
      }
      handleVerifyCode();
    }
  }, [code]);
