...
function HistoryDetailScreen() {
  ...
  const { restaurantId, restaurants } = route.params || {}; // Ensure safe destructuring

  console.log(order);

  // Early return if there's no order data
  if (!order) {
    return (
  ...
  // Safely find the restaurant based on restaurantId
  ...
  if (!restaurant) {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.emptyText}>The restaurant details are missing or invalid.</Text>
      </View>
    );
  }
          ...
          {!isPickup ? (
            <View style={styles.row}>
              ...
              <Text style={styles.value}>
                {order.deliveryScheduledTime ? order.deliveryScheduledTime : '立即配送'}
              </Text>
            </View>
          ) : (
            <View style={styles.row}>
