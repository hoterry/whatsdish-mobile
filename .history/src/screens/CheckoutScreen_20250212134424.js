...
const generateScheduleTimes = () => {
  ...
  <View>
    ...
    <View style={styles.timePickerRow}>
      ...
      <Picker
        selectedValue={pickupScheduledTime ? pickupScheduledTime.toISOString() * 60 + pickupScheduledTime.toISOString() : null}
      ...
      >
        {scheduleTimes.map((time, index) => (
          <Picker.Item key={index} label={time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} value={time.toISOString() * 60 + time.getMinutes()} />
        ))}
      </Picker>
    </View>
  </View>
