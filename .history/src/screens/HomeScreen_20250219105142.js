...
const HomeScreen = () => {
  ...
  const [restaurants, setRestaurants] = useState([]); // 保留所有餐廳資料
  const [filteredRestaurants, setFilteredRestaurants] = useState([]); // 過濾後的餐廳資料
  ...
  const filterRestaurants = (query) => {
    if (!query) {
      ...
      setFilteredRestaurants(filtered); // 更新過濾後的餐廳列表
    }
  };
