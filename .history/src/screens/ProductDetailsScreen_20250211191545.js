...
const handleAddToCart = () => {
  if (selectedOption) {
    ...
    // Create a unique ID by combining menuItem.id, selectedOption.id, and a sorted list of selected modifiers' IDs
    const itemWithOptionAndModifiers = {
      ...menuItem,
      selectedOption,
      selectedModifiers,
      price: selectedOption.price + totalModifiersPrice,  // Include modifier prices in total
      uniqueId: `${menuItem.id}-${selectedOption?.id || 'default'}-${selectedModifiers
        .sort((a, b) => a.id - b.id)  // Sort modifiers to ensure consistency
        .map(m => m.id).join('-')}`,  // Join modifier IDs for uniqueness
    };
