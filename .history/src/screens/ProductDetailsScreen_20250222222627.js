...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (__DEV__) {
      console.log("Options available in Product detail Screen:", options);
      console.log("Modifiers available in Product detail Screen:", modifiers);
    }
  }, []); 
  ...
  useEffect(() => {
    ...
    if (menuItem.modifier_groups && menuItem.modifier_groups.length > 0) {
      menuItem.modifier_groups.forEach((group, index) => {
        if (__DEV__) {
          console.log(`Modifier Group in Product detail Screen ${index + 1}:`, group.modifier_items); // Log modifier_items array inside each group
        }
      });
    }
  ...
  const toggleModifier = (modifier, group) => {
    if (__DEV__) {
      console.log("Toggling modifier in Product detail Screen:", modifier);
    }
