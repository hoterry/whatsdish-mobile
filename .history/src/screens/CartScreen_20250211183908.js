...
function CartScreen() {
  ...
  const renderCartItem = (item) => {
    console.log('Cart Item:', JSON.stringify(item, null, 2)); // 打印购物车里的每个 item
    console.log('Cart Item:', item.selectedOption.name); // 打印购物车里的每个 item
    return (
        ...
        <View style={styles.cartItemDetails}>
          ...
          {item.selectedOption && item.selectedOption.length > 0 && (
            <View style={styles.optionsContainer}>
              ...
              {item.selectedOption.map((selectedOption, index) => (
