...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (__DEV__) {
      console.log("Options available (Product detail Screen):", options);
      console.log("Modifiers available (Product detail Screen):", modifiers);
    }
  }, []); 
  ...
  useEffect(() => {
    ...
    if (menuItem.modifier_groups && menuItem.modifier_groups.length > 0) {
      menuItem.modifier_groups.forEach((group, index) => {
        if (__DEV__) {
          console.log(`Modifier Group (Product detail Screen) ${index + 1}:`, group.modifier_items); // Log modifier_items array inside each group
        }
      });
    }
