...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const options = menuItem.option_groups
    ? option_groups.options.map(option => ({
        id: option.id,
  ...
  const handleAddToCart = () => {
    ...
    const itemWithOptionAndModifiers = {
      ...
      selectedOption,
      selectedModifiers,
      price: (selectedOption ? selectedOption.price : menuItem.price) + totalModifiersPrice,
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={styles.price}>
        ${selectedOption ? selectedOption.price + selectedModifiers.reduce((total, modifier) => total + modifier.price, 0) : menuItem.price}
      </Text>
