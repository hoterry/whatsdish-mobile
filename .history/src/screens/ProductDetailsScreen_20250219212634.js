...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const toggleModifier = (modifier) => {
    ...
    setSelectedModifiers((prevModifiers) => {
      ...
      // Find the group that this modifier belongs to
      const group = menuItem.modifier_groups?.find(group => group.name === modifier.modifierGroupName);
      
      if (!group) {
        console.error(`Group not found for modifier: ${modifier.name}`);
        return prevModifiers; // Return early if no group is found
      }
  
      const { minRequired = 0, maxAllowed = Infinity } = group;  // Fallback to 0 for minRequired, Infinity for maxAllowed
  
      if (isSelected) {
        // If removing, ensure that we are not violating the minRequired constraint
        const selectedGroupModifiers = prevModifiers.filter(mod => mod.modifierGroupName === modifier.modifierGroupName);
        if (selectedGroupModifiers.length <= minRequired) {
          alert(`You must select at least ${minRequired} options for ${group.name}`);
        ...
        const selectedGroupModifiers = prevModifiers.filter(mod => mod.modifierGroupName === modifier.modifierGroupName);
        if (selectedGroupModifiers.length >= maxAllowed) {
          alert(`You can only select up to ${maxAllowed} options for ${group.name}`);
