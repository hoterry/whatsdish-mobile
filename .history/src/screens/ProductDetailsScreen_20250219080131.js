...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const basePrice = menuItem.price_formatted ? menuItem.price_formatted / 100 : 0;  // 確保 menuItem.price 正確
  const optionPrice = selectedOption ? (selectedOption.price ? selectedOption.price / 100 : 0) : 0;  // 確保 option.price 正確
  const totalModifiersPrice = selectedModifiers.reduce((total, modifier) => total + (modifier.price / 100), 0); // 確保 modifier.price 正確
