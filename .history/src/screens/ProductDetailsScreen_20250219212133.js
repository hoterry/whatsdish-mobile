...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const handleAddToCart = () => {
    ...
    for (let group of menuItem.modifier_groups) {
      const selectedGroupModifiers = selectedModifiers.filter((modifier) =>
      ...
      if (selectedGroupModifiers.length < group.minRequired) {
        ...
        return; // 如果有任何 group 不符合 minRequired，則終止加入購物車
      }
    }
  ...
  const toggleModifier = (modifier, group) => {
    ...
    // 獲取選中的 modifiers，根據 minRequired 和 maxAllowed 來控制選擇
    setSelectedModifiers((prevModifiers) => {
      // 如果該 modifier 已經被選中，則移除它
      const isSelected = prevModifiers.some((m) => m.id === modifier.id);
      if (isSelected) {
        // 如果移除後，選擇的數量還小於 minRequired，則不允許移除
        if (prevModifiers.length <= group.minRequired) {
          alert(`You must select at least ${group.minRequired} options.`);
          return prevModifiers;  // 不移除選項
        }
        return prevModifiers.filter((m) => m.id !== modifier.id);
      } else {
        // 如果選擇後，數量超過 maxAllowed，則不允許再選擇
        if (prevModifiers.length >= group.maxAllowed) {
          alert(`You can only select up to ${group.maxAllowed} options.`);
          return prevModifiers;  // 不添加選項
        }
