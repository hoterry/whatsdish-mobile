...
export default function LoginScreen({ setIsAuthenticated }) {
  ...
  const [autoCode, setAutoCode] = useState('');  // For receiving auto-filled code
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setIsCodeSent(true);
        // Simulating receiving an auto-filled code (for testing)
        // Uncomment below line when integrating with actual SMS API that provides auto-fill
        // setAutoCode(data.autoCode); // assuming the API returns the code
      } else {
  ...
  useEffect(() => {
    if (autoCode) {
      // Automatically fill the code once it's received
      setCode(autoCode.split(''));  // Assumes autoCode is a string of 6 digits
    }
  }, [autoCode]);
