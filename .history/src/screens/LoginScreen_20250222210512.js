...
export default function LoginScreen({ setIsAuthenticated }) {
  ...
  const handleSendCode = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/send-code`, { // 使用從 manifest 中讀取的 API_URL
        method: 'POST',
        headers: {
  ...
  const handleVerifyCode = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/verify-code`, { // 使用從 manifest 中讀取的 API_URL
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
