...
import LocationFetcher from '../context/LocationFetcher'; // Import the LocationFetcher component
  ...
  useEffect(() => {
    const logToken = async () => {
      try {
        ...
        if (token) {
          console.log('User Token in HomeScreen:', token);
        } else {
  ...
  useEffect(() => {
    ...
    const loadSavedLanguage = async () => {
      try {
        ...
        if (savedLanguage) {
          changeLanguage(savedLanguage); // Change language if found in AsyncStorage
        }
      } catch (error) {
          ...
          <View style={styles.topBarRow}>
            <TouchableOpacity style={styles.location}>
              <Text style={styles.locationText} numberOfLines={1}>
                {location || 'Fetching location...'} {/* Display location */}
              </Text>
