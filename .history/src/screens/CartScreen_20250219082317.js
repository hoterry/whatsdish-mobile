...
function CartScreen({ route }) {
  ...
  useEffect(() => {
    console.log('Received restaurants data IN cart:', restaurants); // 打印收到的 restaurants 數據
  }, [restaurants]);
  ...
  const renderCartItem = (item) => {
    ...
    // 確保價格是有效的數字
    const basePrice = item.price_in_cents ? item.price_in_cents / 100 : 0;
    const optionPrice = item.selectedOption?.price ? item.selectedOption.price / 100 : 0;
    const modifiersPrice = item.selectedModifiers?.reduce((total, modifier) => total + (modifier.price / 100), 0) || 0;
    const totalPrice = (basePrice + optionPrice + modifiersPrice) * item.quantity;
