...
function CartScreen() {
  ...
  const handleIncreaseQuantity = (uniqueId) => {
    const item = safeCart.find(item => item.uniqueId === uniqueId); // Using uniqueId to find the item
    if (item) {
  ...
  const handleDecreaseQuantity = (uniqueId) => {
    ...
    if (item && item.quantity > 1) {
      updateQuantity(restaurantId, item.uniqueId, item.quantity - 1); // Using uniqueId to update the quantity
    }
  };

  const renderCartItem = (item) => {
    // Ensure each item has a unique key (either use `uniqueId` or fallback to a combination of properties)
    const key = item.uniqueId || `${item.name}-${item.price}-${item.quantity}`; // Fallback to a unique combination if `uniqueId` is missing
        ...
        <View style={styles.cartItemDetails}>
          ...
          {item.selectedOption && (
            <View style={styles.selectedOptionContainer}>
              <Text style={styles.selectedOptionTitle}>Size:</Text>
